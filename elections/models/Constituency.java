...
public class Constituency {
    ...
    public boolean register(int rank, Candidate candidate) {
        NavigableMap<Integer, Candidate> party = rankedCandidatesByParty.computeIfAbsent(
                candidate.getParty(),
                q -> new TreeMap(Map.of(rank, candidate))
        );

        if (party.containsKey(rank) || party.containsValue(candidate)) return false;

        party.put(rank, candidate);

        rankedCandidatesByParty.put(candidate.getParty(), party);
    ...
    public Candidate getCandidate(Party party, int rank) {
        // return the candidate at the given rank in the given part
        NavigableMap<Integer, Candidate> foundParty = rankedCandidatesByParty.get(party);
        if (foundParty == null) return null;

        return foundParty.get(rank);
    ...
    public final List<Candidate> getCandidates(Party party) {
        ...
        return rankedCandidatesByParty.get(party).entrySet().stream()
    ...
    public Set<Candidate> getAllCandidates() {
        ...
        return rankedCandidatesByParty
                .entrySet()
                .stream()
                .flatMap(b -> b.getValue().values().stream())
    ...
    public NavigableSet<PollingStation> getPollingStationsByZipCodeRange(String firstZipCode, String lastZipCode) {
        ...
        // right:    1234AB | 1234 AB | 1001 AB
        // wrong:  0123AB | 1234A B | 0123 AB

        String zipcodeRegex = "^[1-9][0-9]{3}\s?[a-zA-Z]{2}$";

        return pollingStations.stream()
                .filter(p -> p.getZipCode().matches(zipcodeRegex))
                .filter(p -> p.getZipCode().compareTo(lastZipCode) <= 0)
                .filter(p -> p.getZipCode().compareTo(firstZipCode) >= 0)

                // Convert stream to map with a comparator that compares the hash code
                .collect(Collectors.toCollection(
                        () -> new TreeSet<>(Comparator.comparingLong(PollingStation::hashCode))
                ));
    }
    ...
    public Map<Party, Integer> getVotesByParty() {
        ...
        return pollingStations.stream()
                .flatMap(p-> p.getVotesByParty().entrySet().stream())
                .collect(Collectors.groupingBy(Map.Entry::getKey, Collectors.summingInt(Map.Entry::getValue))); // replace by a proper outcome
    }
