...
public class Constituency {
    ...
    public Constituency(int id, String name) {
        this.id = id;
        this.name = name;
        //  initialise this.rankedCandidatesByParty with an appropriate Map implementation
    ...
    public boolean register(int rank, Candidate candidate) {
        // Check if a map of registered candidates already exists for the party of the given candidate
        NavigableMap<Integer, Candidate> registeredCandidates = rankedCandidatesByParty.computeIfAbsent(candidate.getParty(), k -> new TreeMap<>());

        // Check if the given rank is already taken by another candidate or if the candidate is already registered at another rank
        if (registeredCandidates.containsKey(rank) || registeredCandidates.containsValue(candidate)) {
            return false;
        }
        // Register the candidate at the given rank
        registeredCandidates.put(rank, candidate);
    ...
    public Candidate getCandidate(Party party, int rank) {
        ...
        Map<Integer, Candidate> registeredCandidates = rankedCandidatesByParty.get(party);
        if (registeredCandidates != null){
            // filtering candidates and find the candidate at the given rank
            return registeredCandidates.entrySet().stream()
                    .filter(entry -> entry.getKey() == rank)
    ...
    public final List<Candidate> getCandidates(Party party) {
        ...
        Map<Integer, Candidate> partyCandidateMap = rankedCandidatesByParty.get(party);

        if (partyCandidateMap != null) {
            // Create a sorted list of candidates based on their rank
            return partyCandidateMap.entrySet().stream()
                    .sorted(Map.Entry.comparingByKey())
                    .map(Map.Entry::getValue)
                    .collect(Collectors.toList());
        }
        // No registered candidates for the given party
        return Collections.emptyList(); // replace by a proper outcome
    }
    ...
    public Map<Party,Integer> getVotesByParty() {
        // prepare a map of total number of votes per party
        return pollingStations.stream()
                .flatMap(ballotBox -> ballotBox.getVotesByCandidate().entrySet().stream())
